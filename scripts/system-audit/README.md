...
```bash
chmod +x system-audit.sh    # one-time, makes the script executable
./system-audit.sh
```
...
The script uses `set -euo pipefail` for fail-fast behavior. Optional commands (`last`, `hostnamectl`) are checked with `command -v` before running — if missing, the section is skipped with a note rather than crashing the whole audit.
